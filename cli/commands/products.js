// AI 生成，手动调整：添加 remove 命令、var → const/let
...
function register(cmd) {
  cmd
    ...
    .action(function (opts) {
      const params = { page: parseInt(opts.page), limit: parseInt(opts.limit) };
  ...
  cmd
    .command('remove <id>')
    .description('强制下架商品（需管理员 Token）')
    .requiredOption('-r, --reason <reason>', '下架原因')
    .action(function (id, opts) {
      apiClient
        .put('/admin/products/' + id, { status: 'removed' })
        .then(function (res) {
          console.log(chalk.green('✔ ' + res.data.message));
          console.log('  下架原因: ' + opts.reason);
        })
        .catch(function (err) {
          handleError(err);
        });
  ...
  cmd
    ...
    .action(function (keyword) {
      apiClient
        .get('/products', { params: { keyword: keyword, page: 1, limit: 20 } })
        .then(function (res) {
          const data = res.data;
          if (data.products.length === 0) {
