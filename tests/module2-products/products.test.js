...
describe('GET /api/products', function () {
  ...
  it('search搜索返回匹配结果', async function () {
    var res = await request('GET', '/products?keyword=Python');
    assert.strictEqual(res.status, 200);
  ...
  it('sort=price_asc升序排列正确', async function () {
    var res = await request('GET', '/products?sort=price_asc');
    assert.strictEqual(res.status, 200);
  ...
  it('sort=price_desc降序排列正确', async function () {
    var res = await request('GET', '/products?sort=price_desc');
    assert.strictEqual(res.status, 200);
    var prices = res.body.products.map(function (p) {
      return p.price;
    });
